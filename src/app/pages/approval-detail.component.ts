...
export class ApprovalDetailComponent {
  ...
  rows: DetailRow[] = [
    ...
    { label: 'Technical Specification', value: '--' },
    ...
    { label: 'Proposed Signal Character', value: '--' },
    { label: 'Proposed Nominal Range', value: '--' },
    { label: 'Names, Letters or Numbers', value: '--' },
  ...
  onSpecificationsSaved(payload: any): void {
    ...
    if (first) {
      this.patch('Technical Specification', first.navigationAidType || '--');
      ...
      this.patch(
        'Colour of Structure',
        first.structureColour === 'أخرى'
          ? first.structureColourOther || 'أخرى'
          : first.structureColour || '--'
      );
      this.patch('Proposed Signal Character', first.signalCharacter || '--');
      this.patch('Proposed Nominal Range', first.nominalRange ? first.nominalRange + ' NM' : '--');
      this.patch('Names, Letters or Numbers', first.nationalNumbers || '--');
